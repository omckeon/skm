...
if [ "$SK_OS" = "macos" ]; then
    ...
    if command -v python3 &> /dev/null; then
        ...
        # echo "For Python support: Please install python3, then run this script again."
        # Could potentially just install it as below?
        echo "Installing python..."
        pacman -S mingw-w64-x86_64-python --noconfirm --disable-download-timeout
    fi
else
...
if [ "$SK_OS" = "linux" ]; then
    ...
    pacman -S --needed --noconfirm --disable-download-timeout mingw-w64-x86_64-clang mingw-w64-x86_64-gcc mingw-w64-x86_64-gdb mingw-w64-x86_64-cmake mingw-w64-x86_64-SDL2 mingw-w64-x86_64-SDL2_mixer mingw-w64-x86_64-SDL2_image mingw-w64-x86_64-SDL2_ttf mingw-w64-x86_64-SDL2_net mingw-w64-x86_64-civetweb
fi
